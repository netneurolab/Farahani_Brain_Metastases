...
Code of the verification:
    ...
    Lung cancer data from another sample

    spearmanr = 0.5589481646599473
...
cancer_name = 'Lung_distribution_HEAD-MNI09aSym.nii'
cancer_name_short = 'Lung_distribution_HEAD-MNI09aSym_surf'
...
BM_parcellated = convert_cifti_to_parcellated_SchaeferTian(data,
                                                           'cortex', 'an',
                                                           path_results, cancer_name_short + '_400')
...
BM_img = nib.load(path_validation + 'Lung_distribution_HEAD.nii')
...
np.save(path_results + 'replication_Lung_HEAD.npy', verification_data)

#------------------------------------------------------------------------------
# Load main data
#------------------------------------------------------------------------------

main_nulls = np.load(path_results + 'nulls_Lung_10k_surf_brainspace.npy')[:1000,:]
main_cancer_parc_cereb = np.load(path_results + 'Lung_34_cerebellum.npy')
main_cancer_parc_cortex = np.load(path_results + 'Lung_400_surf.npy').flatten()
...
plt.tight_layout()
plt.savefig(path_results + "validation_Lung.svg", format="svg")
plt.show()
